...
KEY_COORDS = {
    ...
    'a': (0.25, 1), 's': (1.25, 1), 'd': (2.25, 1), 'f': (3.25, 1), 'g': (4.25, 1), 'h': (5.25, 1), 'j': (6.25, 1), 'k': (7.25, 1), 'l': (8.25, 1), ';': (9.25, 1),
    'z': (0.75, 2), 'x': (1.75, 2), 'c': (2.75, 2), 'v': (3.75, 2), 'b': (4.75, 2), 'n': (5.75, 2), 'm': (6.75, 2), ',': (7.75, 2), '.': (8.75, 2), '/': (9.75, 2),
    # 特殊キーはホームポジションから遠い位置として仮置
    'tab': (-1, 0), 'caps': (-1, 1), 'lshift': (-1, 2),
    'bs': (10, 0), 'enter': (10, 1), 'rshift': (10, 2)
}
...
class PathNode:
    ...
    def __init__(self, cost: float, count: int, logs: List[str], prev: Optional['PathNode']):
        self.cost = cost
        self.count = count
        self.logs = logs
        self.prev = prev
...
class TypingCostCalculator:
    def __init__(self, layout: str, verbose: bool = False, cost_mode: str = "default"):
        ...
        base_to_layout_map = {BASE_CHARS[i]: parsed_layout[i] for i in range(min(len(BASE_CHARS), len(parsed_layout)))}
        ...
        for key, hand, finger in zip(QWERTY_KEYS, LEFT_HAND_MAP, FINGERS_MAP):
            ...
            self.layout_map[layout_char].append({'base': key, 'hand': hand, 'finger': finger})
        ...
        if self.cost_mode == "calculated":
            ...
            if self.verbose: print("[Info] Using Dynamically Calculated Cost Matrix.")
        else:
            self.cost_matrix = DEFAULT_COST_MATRIX
            if self.verbose: print("[Info] Using Default Hardcoded Cost Matrix.")
    ...
    def _generate_calculated_matrix(self) -> dict:
        ...
        """
        calculated_matrix = {}
        # 指ごとの重み (人差し指を基準とする。薬指は器用さの欠如、小指は筋力弱さを反映)
        finger_weights = {
            0: 1.3,  # 小指 (Pinky)
            1: 1.6,  # 薬指 (Ring)
            2: 1.1,  # 中指 (Middle)
        ...
        base_cost_multiplier = 15.0 # スケール合わせのための係数

        for from_key in DEFAULT_COST_MATRIX.keys():
            cost_list = []
            
            # 左手の18キー + 右手の18キー の順にリストを作る (元の仕様に準拠)
            for to_key in (self.base_keys_left + self.base_keys_right):
                if from_key not in KEY_COORDS or to_key not in KEY_COORDS:
                    cost_list.append(50) # 座標不明の特殊キー等は固定値
                    continue
                
                # 到達先キーの指と、直前のキーの指を取得
                target_finger = self.base_to_finger_map[to_key]
                from_finger = self.base_to_finger_map.get(from_key, target_finger) # 特殊キーからの場合はとりあえず同一指扱いしない等も可
                
                # 1. 直前のキーからの移動距離 (Travel Distance)
                # ★同じ指が連続して動く場合のみ距離を計算し、違う指の場合は各々のホームポジションから動くとみなす（移動距離0）
                if from_finger == target_finger:
                    x1, y1 = KEY_COORDS[from_key]
                    x2, y2 = KEY_COORDS[to_key]
                    travel_distance = math.sqrt((x2 - x1)**2 + (y2 - y1)**2)
                else:
                    travel_distance = 0.0
                
                # 2. ホームポジション(y=1)からの距離 (Base Distance)
                base_distance = abs(KEY_COORDS[to_key][1] - 1.0)
                
                # 指の弱さペナルティ
                weight = finger_weights.get(target_finger, 1.0)
                
                # 移動距離とベース距離を合成してコスト化（移動距離の荷重を 3.0倍 に引き上げ）
                calculated_cost = int((travel_distance * 3.0 + base_distance * 1.5 + 0.2) * weight * base_cost_multiplier)
                
                # 同じキーの連続は移動距離0だが、最低コストを保証
                if from_key == to_key:
                    calculated_cost = int((base_distance * 1.5 + 0.2) * 10 * weight)
    ...
    def _get_cost(self, from_base: str, to_base: str, to_hand: int) -> int:
        try:
            if from_base not in self.cost_matrix: return 0
            cost_list = self.cost_matrix[from_base]
            target_index = self.cost_index_left[to_base] if to_hand == 1 else self.cost_index_right[to_base] + len(self.base_keys_left)
            # ↑ 右手のインデックス取得時、リスト後半(18〜35番目)を参照するように修正
            if to_hand == 0: 
                target_index = self.cost_index_right[to_base]
                # Defaultマトリクスは各行18要素（打つ手のみ）だが、
                # もし36要素連結で作った場合はインデックス調整が必要。
                # 現状は元の「各手18要素ずつ格納」の仕様に合わせる
            
            return cost_list[target_index]
        except (KeyError, IndexError):
            return 0

    def calculate(self, text: str, sfb_base_penalty: int = 80, scissor_penalty: int = 50, 
                  row_jump_penalty: int = 30, inward_roll_bonus: int = 5, outward_roll_bonus: int = 2, 
                  alternation_bonus: int = 3) -> Tuple[float, float, int]:
        
        initial_state = DpState(self.initial_left_base, self.initial_right_base, None, None, None, self.initial_left_char, self.initial_right_char)
        dp = collections.defaultdict(dict)
        dp[0] = {initial_state: PathNode(0, 0, [], None)}
        processed_text = text.replace('\r\n', '\n').replace('\r', '\n')
        total_len = len(processed_text)
        layout_keys_str = [k for k in self.layout_map.keys() if k not in ['enter', 'tab']]
        ...
        for i in range(total_len):
            ...
            if char == '\n' and 'enter' in self.layout_map: matches.append(('enter', 1, False, 'enter'))
            elif char == '\t' and 'tab' in self.layout_map: matches.append(('tab', 1, False, 'tab'))
            else:
                for key_str in layout_keys_str:
                    match_len = len(key_str)
                    if i + match_len <= total_len and processed_text[i:i+match_len].lower() == key_str:
                        matches.append((key_str, match_len, processed_text[i:i+match_len][0].isupper(), processed_text[i:i+match_len]))
            ...
            for state, node in dp[i].items():
                for key_str, match_len, is_upper, display_char in matches:
                    for cand in self.layout_map[key_str]:
                        cand_base_key, cand_hand, cand_finger = cand['base'], cand['hand'], cand['finger']
                        ...
                        c_hand, c_finger, c_base = state.last_hand, state.last_finger, state.last_base_key

                        if is_upper:
                            s_hand = 0 if cand_hand == 1 else 1
                            s_base = self.right_shift_base if s_hand == 0 else self.left_shift_base
                            last_s_base = c_l_base if s_hand == 1 else c_r_base
                            s_cost = self._get_cost(last_s_base, s_base, s_hand)
                            step_cost += max(0, s_cost)
                            
                            c_hand, c_finger, c_base = s_hand, 0, s_base
                            if s_hand == 1: c_l_base = c_l_char = s_base
                            else: c_r_base = c_r_char = s_base

                        last_m_base = c_l_base if cand_hand == 1 else c_r_base
                        main_cost = self._get_cost(last_m_base, cand_base_key, cand_hand)
                        ...
                        if c_hand == cand_hand:
                            ...
                            if c_finger == cand_finger and c_base != cand_base_key:
                                ...
                                # 薬指と小指は独立性ペナルティを重くする
                                weakness_multiplier = 1.5 if cand_finger in [0, 1] else 1.0
                                distance_penalty = abs(cur_row - cand_row) * 10 
                                
                                sfb_total = (sfb_base_penalty + distance_penalty) * weakness_multiplier
                                main_cost += sfb_total
                                if self.verbose: step_logs.append(f"  -> SFB Penalty (+{sfb_total})")
                                ...
                                if abs(cur_row - cand_row) == 2:
                                    ...
                                    if self.verbose: step_logs.append(f"  -> Row Jump Penalty (+{row_jump_penalty})")

                                # 3. Scissors
                                if c_finger is not None and abs(c_finger - cand_finger) == 1 and cur_row != cand_row:
                                    main_cost += scissor_penalty
                                    if self.verbose: step_logs.append(f"  -> Scissors Penalty (+{scissor_penalty})")

                                # 4. Rolls (隣り合う指のみ)
                                if c_finger is not None and abs(c_finger - cand_finger) == 1:
                                    if c_finger < cand_finger:
                                        main_cost -= inward_roll_bonus
                                        if self.verbose: step_logs.append(f"  -> Inward Roll (-{inward_roll_bonus})")
                                    else:
                                        main_cost -= outward_roll_bonus
                                        if self.verbose: step_logs.append(f"  -> Outward Roll (-{outward_roll_bonus})")
                        else:
                            if c_hand is not None:
                                main_cost -= alternation_bonus
                                if self.verbose: step_logs.append(f"  -> Alternation Bonus (-{alternation_bonus})")

                        main_cost = max(0, main_cost)
                        step_cost += main_cost
                        step_count += match_len

                        if cand_hand == 1: c_l_base, c_l_char = cand_base_key, key_str
                        else: c_r_base, c_r_char = cand_base_key, key_str

                        new_state = DpState(c_l_base, c_r_base, cand_hand, cand_finger, cand_base_key, c_l_char, c_r_char)
                        target_i = i + match_len
                        new_cost = node.cost + step_cost
                        
                        if new_state not in dp[target_i] or dp[target_i][new_state].cost > new_cost:
                            dp[target_i][new_state] = PathNode(new_cost, node.count + step_count, step_logs if self.verbose else [], node)
            del dp[i]

        states = dp[len(processed_text)]
        if not states: return 0, -20, 0

        best_node = min(states.values(), key=lambda n: n.cost)
        return best_node.cost, (best_node.cost / best_node.count if best_node.count > 0 else 0), best_node.count

def evaluate_layout(name: str, layout: str, text_to_check: str, cost_mode: str = "default", norm: float = 1.0) -> float:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ※以前のコードに合わせて calcn を使用しています。
        # evaluate_layout に変更している場合は適宜関数名を変更してください。
        qwerty_norm = evaluate_layout("QWERTY", qwerty_layout, text_to_check, cost_mode, norm=1.0)
...
#            ("kotone5", 'qwrdfjluyxnstagcaieozpmhbkv,./')
        ...
        for name, layout in layouts_to_test:
            evaluate_layout(name, layout, text_to_check, cost_mode, norm=qwerty_norm)
